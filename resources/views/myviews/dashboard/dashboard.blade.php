        ...
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex flex-column align-items-center gap-1">
              <h2 class="mb-2">
                @php($qtw = 0)
                @foreach($weeklySales as $rs)
                  @php( $qtw = $rs->quantity + $qtw )
                @endforeach
                {{$qtw}}
              </h2>
                  ...
                  <div class="me-2">
                    ...
                    <small class="text-muted">
                      ...
                      @foreach($products as $product)
                        {{$product->title}}
                        @php($data = \App\Models\product_shopcart::where('product_id',$product->id)->sum('quantity'))
                        @php($qtc = $data + $qtc)
                      @endforeach
                    </small>
                  </div>
                  <div class="user-progress">
                    <small class="fw-medium">{{$qtc}}</small>
                  </div>
                </div>
              </li>
            @endforeach
